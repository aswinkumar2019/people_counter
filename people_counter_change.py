...
x_range = np.arange(x_start-0.7,x_end+0.7,0.1)
x_range = np.round(x_range,1).tolist()
y_range = np.arange(y_start-0.7,y_end+0.7,0.1)
...
while True:
    ...
    if shared["in_type"] == 'file' is not None and frame is None:
        break
    ...
    # object *detectors* to obtain a higher frame processing throughput
        ...
        for tracker in trackers:
            ...
            if(x_range.count(center[0]) >= 1 and y_range.count(center[1]) >= 1):
    ...
    for (objectID, centroid) in objects.items():
        ...
        # to determine direction
            ...
            print("x:",x)
            print("y:",y)
            if(x_range.count(x) == 1 and y_range.count(y) == 1):
                cross_x = cross_x + 1
                print("Object has crossed the line")
                sleep(5)
